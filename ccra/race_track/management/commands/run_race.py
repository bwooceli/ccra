...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if race_data_output_file_name == "":
            unit_name = input("        Enter unit name (eg 'Pack 3456', default is '3456): ")
            if unit_name == "":
                unit_name = "3456"

            race_type = input("Enter race type (eg 'Cubs' or 'Outlaw', default is 'Cubs'): ")
            if race_type == "":
                race_type = "Cubs"

            current_date_string = datetime.datetime.now().strftime("%Y-%m-%d")
            
            race_data_output_file_name = f'{current_date_string}_{unit_name}_{race_type}.csv'

        if starting_car_number > ending_car_number:
            raise CommandError("Starting car number must be less than ending car number")
