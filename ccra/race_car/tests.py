...
class TestRaceEvent(TestCase):
    ...
    def test_save_car_maker_saves_correctly(self):
        ...
        self.assertEqual(
            f"{first_name_without_spaces} {last_name_without_spaces}",
            str(car_maker_child),
        )

        car_maker_parent = car_models.CarMaker.objects.create(user=self.test_user)
        ...
        self.assertEqual(
            f"{car_maker_child.first_name} {car_maker_child.last_name} (Helper: {self.test_user})",
            str(car_maker_child),
        )
